...
const ViewPaste = () => {
  ...
  try {
    ...
    if (!res.ok) {
      // 👇 Show error if status is not 2xx
      return alert(`❌ ${data.error || "Something went wrong!"}`);
    }

    alert(` Deleted: ${data.title}`);
    // navigate or refresh the list if needed
  } catch (err) {
 ...
 <p className='text-gray-200 text-lg  self-start whitespace-pre-line'>
  ...
  Created by: {console.log(paste.owner)}{paste.owner?.name || "Unknown"}
</p>
  ...
  <li className="px-4 py-2 hover:bg-blue-600 transition-colors rounded-l-md cursor-pointer font-medium"
  onClick={() => navigate(`/edit/${paste._id}`)}
  >
    ✏️ Edit
  </li>
  <li className="px-4 py-2 hover:bg-red-600 transition-colors cursor-pointer font-medium" 
